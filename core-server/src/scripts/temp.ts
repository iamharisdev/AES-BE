...
// 9f450220-13e3-4011-89f5-db6712953911
// +923184526509
...
async function duplicateCountPerPhone() {
  try {
    const duplicates = await db
      ...
      .from(patient)
      .groupBy(patient.phoneNumber)
      .having(sql`COUNT(*) > 1`) // sirf duplicate phone numbers

    console.log("Duplicate phone numbers with count:",duplicates.length);
    duplicates.forEach(d => {
      console.log(`Phone: ${d.phone_number}, Patients: ${d.count}`);
    });
  } catch (err) {
    console.error("Error fetching duplicate counts:", err);
  }
}

duplicateCountPerPhone();
