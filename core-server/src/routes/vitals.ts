...
// 3. Handlers
const createVitalsHandler = () => {
  app.openapi(createVitalsRoute, async c => {
...
const listVitalsHandler = () => {
  app.openapi(listVitalsRoute, async c => {
...
const getVitalsByIdHandler = () => {
  app.openapi(getVitalsByIdRoute, async c => {
...
const updateVitalsHandler = () => {
  app.openapi(updateVitalsRoute, async c => {
    const { id } = c.req.valid('param');
    const details = c.req.valid('json');
    const record = await db
      .select()
...
const deleteVitalsHandler = () => {
  app.openapi(deleteVitalsRoute, async c => {
    const { id } = c.req.valid('param');
    const record = await db
      .select()
      .from(tables.vitals)
...
export { createVitalsHandler, listVitalsHandler, getVitalsByIdHandler, updateVitalsHandler, deleteVitalsHandler }
