...
export const getAllEmrsFromPhone = () => {
	app.openapi(route, async (c) => {
		...
		// Fetch proposed plans for all EMRs
		const emrIds = emrs.map(emr => emr.emrId)
		const proposedPlans = await db
			.select()
			.from(table.proposedPlan)
			.where(sql`${table.proposedPlan.emrId} IN (${emrIds.join(',')})`)
			.execute()

		// Group proposed plans by emrId
		const proposedPlansByEmrId = proposedPlans.reduce((acc, plan) => {
			if (!acc[plan.emrId]) {
				acc[plan.emrId] = []
			}
			acc[plan.emrId].push(plan)
			return acc
		}, {} as Record<string, typeof proposedPlans>)

		// Add proposed plans to each EMR
		const emrsWithProposedPlans = emrs.map(emr => ({
			...emr,
			proposedPlan: proposedPlansByEmrId[emr.emrId] || []
		}))
		...
		return c.json({ emrs: emrsWithProposedPlans, prevPregnancies }, 200)
	})
}
